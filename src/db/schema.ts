...
export const encuentros = sqliteTable('encuentros', {
  ...
  status: text('status', { enum: ['draft', 'published', 'cancelled'] }).notNull().default('published'),
...
export const productos = sqliteTable('productos', {
  ...
  condition: text('condition', { enum: ['nuevo', 'como_nuevo', 'usado'] }).notNull().default('nuevo'),
  ...
  imageKey: text('image_key'),
  status: text('status', { enum: ['available', 'sold_out', 'draft'] }).notNull().default('available'),
  createdBy: text('created_by')
    .notNull()
    .references(() => users.id, { onDelete: 'restrict' }),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull(),
